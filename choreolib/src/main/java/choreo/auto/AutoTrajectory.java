...
public class AutoTrajectory {
  ...
  private void cmdExecute() {
    ...
    double sampleTime = Math.min(getTrajectoryTime(), trajectory.getTotalTime());
    var sampleOpt = trajectory.sampleAt(sampleTime, allianceCtx.doFlip());
    if (sampleOpt.isEmpty()) {
      return;
    }
    var sample = sampleOpt.get();
  ...
  private Trigger timeTrigger(
      double targetTime, DoubleSupplier timeSupplier, BooleanSupplier isRunningSupplier) {
    ...
    return new Trigger(
        ...
        new BooleanSupplier() {
          ...
          public boolean getAsBoolean() {
            ...
            double nowTimestamp = timeSupplier.getAsDouble();
            try {
  ...
  public Trigger doneDelayed(double seconds) {
    return timeTrigger(seconds, inactiveTimer::get, inactiveTimer::isRunning)
        .and(new Trigger(routine.loop(), () -> isCompleted));
  }
  ...
  public Trigger atTime(double timeSinceStart) {
    ...
    return timeTrigger(timeSinceStart, this::getTrajectoryTime, activeTimer::isRunning);
  }
