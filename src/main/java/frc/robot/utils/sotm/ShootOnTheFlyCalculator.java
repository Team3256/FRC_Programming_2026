...
import frc.robot.utils.sotm.BallPhysics.ShotSolution;
...
public class ShootOnTheFlyCalculator {
  /**
   * Calculates the time it will take for a projectile to reach a target given the robot's pose and
   * the target's pose, and a function describing the projectile's velocity. This allows you to have
   * a shooter that may shoot a projectile at varying speeds given varying distances from the targe.
   *
   * @see #calculateEffectiveTargetLocation(Pose2d, Pose3d, ChassisSpeeds, ChassisAccelerations,
  ...
  public static InterceptSolution solveShootOnTheFly(
    ...
    ShotSolution sol = BallPhysics.solveBallisticWithSpeed(shooterPose, targetPose, targetSpeedRps);
    ...
    for (int i = 0; i < maxIterations; i++) {
      ...
      ShotSolution newSol =
          BallPhysics.solveBallisticWithSpeed(shooterPose, effectiveTarget, targetSpeedRps);
